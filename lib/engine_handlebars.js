...
var normalizedPath = path.join(__dirname, "helpers");

fs.readdirSync(normalizedPath).forEach(function(file) {
  if (!fs.lstatSync(path.join(__dirname, "helpers", file)).isDirectory() &&
    file !== "index.js") {
    require("./helpers/" + file).init(Handlebars);
  }
});
...
var engine_handlebars = {
  ...
  registerPartial: function (pattern) {
    // instead of using the patternLabName, we use the file-name as the partial key
    // Handlebars.registerPartial(pattern.patternPartial, pattern.template);
    Handlebars.registerPartial(pattern.relPath.replace(/\.hbs/, ''), pattern.template);
  },
